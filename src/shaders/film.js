...
float grain(vec2 texCoord, vec2 resolution, float frame, float multiplier) {
  ...
  vec2 safeTC = max(texCoord, vec2(0.001));
  float n1 = pnoise3D(vec3(mult, offset), vec3(1.0 / safeTC * resolution, 1.0));
  return n1 / 2.0 + 0.5;
}
...
void main() {
  ...
  float dim = 1024.0;
  float zoom = 0.35;
  vec3 g = vec3(grain(vTexCoord, vec2(dim, dim) * zoom, time, 2.5));
  float luminance = luma(texColor);
  vec3 color = blendSoftLight(texColor, g);
  float response = smoothstep(0.05, 0.5, luminance);
  color = mix(color, toneMapAces(texColor), pow(response, 2.0));
